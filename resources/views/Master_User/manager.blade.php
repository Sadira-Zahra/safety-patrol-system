    ...
    <div class="col-12">
      ...
      <div class="card card-outline card-primary compact-card">
        <div class="card-header">
          <h3 class="card-title">
            <i class="fas fa-user-tag mr-2"></i>Manager Departemen
          </h3>
          <div class="card-tools">
            <a href="#" class="btn btn-light btn-sm" data-toggle="modal" data-target="#modalAdd">
              <i class="fas fa-plus mr-1"></i>Tambah Manager Departemen
            </a>
          </div>
        </div>
                  ...
                  <tr>
                    ...
                    <td class="text-center text-nowrap">
                      <a href="#" class="btn btn-warning btn-sm btn-edit"
                         data-show="{{ route('dept-managers.show', $user->id) }}"
                         data-update="{{ route('dept-managers.update', $user->id) }}">
                        <i class="fas fa-edit"></i>
                      </a>
                      <form action="{{ route('dept-managers.destroy', $user->id) }}" method="POST" class="d-inline"
    ...
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">
          <i class="fas fa-user-plus mr-2"></i>Tambah Manager Departemen
        </h5>
        <button type="button" class="close text-white" data-dismiss="modal"><span>&times;</span></button>
      </div>

      <form action="{{ route('dept-managers.store') }}" method="POST">
        @csrf
...
<div class="modal fade" id="modalEdit" tabindex="-1">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">
          <i class="fas fa-edit mr-2"></i>Edit Data Manager Departemen
        </h5>
        <button type="button" class="close text-white" data-dismiss="modal"><span>&times;</span></button>
      </div>
    ...
    $(function () {
      ...
      $(document).on('click', '.btn-edit', function (e) {
        ...
        $.get(showUrl, function (res) {
          $('#edit_nik').val(res.nik);
          $('#edit_name').val(res.name);
          $('#edit_email').val(res.email);
          $('#edit_departemen_id').val(res.departemen_id);
